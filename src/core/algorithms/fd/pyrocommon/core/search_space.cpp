...
std::optional<DependencyCandidate> SearchSpace::PollLaunchPad() {
    while (true) {
        ...
        spdlog::trace((boost::format{"* Escaping launch_pad %1% from: %2%"} %
                              launch_pad.vertical_.ToString() % "[UNIMPLEMENTED]").str());
...
void SearchSpace::EscapeLaunchPad(Vertical const& launch_pad,
    ...
    for (auto& escaping : hitting_set) {
        ...
        spdlog::trace((boost::format{"  Escaped: %1%"} % escaped_launch_pad_vertical.ToString()).str());
        spdlog::debug((boost::format{"  Proposed launch pad arity: %1% should be <= max_lhs: %2%"} %
                              escaped_launch_pad.vertical_.GetArity() %
                              context_->GetParameters().max_lhs).str());
...
bool SearchSpace::Ascend(DependencyCandidate const& launch_pad) {
    ...
    spdlog::debug((boost::format{"===== Ascending from %1% ======"} %
                          strategy_->Format(launch_pad.vertical_)).str());
        ...
        if (traversal_candidate.IsExact()) {
            ...
            if (traversal_candidate.error_.GetMin() > strategy_->max_dependency_error_) {
                spdlog::trace((boost::format{"  Skipping check form %1% (estimated error: %2%)."} %
                                      ...
                                      traversal_candidate.error_).str());
                error.reset();
            } else {
    ...
    if (!error) {
        spdlog::trace((boost::format{"  Hit ceiling at %1%."} %
                              traversal_candidate.vertical_.ToString()).str());
    ...
    if (*error <= strategy_->max_dependency_error_) {
        spdlog::trace((boost::format{"  Key peak in climbing phase e(%1%)=%2% -> Need to minimize."} %
                           traversal_candidate.vertical_.ToString() % *error).str());
...
void SearchSpace::TrickleDown(Vertical const& main_peak, double main_peak_error) {
    spdlog::debug((boost::format{"====== Trickling down from %1% ======"} % main_peak.ToString()).str());
        ...
        if (!subset_deps.empty()) {
            ...
            for (auto& escaped_peak_vertical : escaped_peak_verticals) {
                if (escaped_peak_vertical.GetArity() > 0 &&
                    ...
                    spdlog::trace("CreateDependencyCandidate as an escaped peak while trickling down");
    ...
    spdlog::debug((boost::format{"* %1% alleged minimum dependencies (%2%)"} %
                          alleged_min_deps->GetSize() % "UNIMPLEMENTED").str());
    ...
    for (auto& [alleged_min_dep, info] : alleged_min_deps->EntrySet()) {
        if (info->is_extremal_ && !global_visitees_->ContainsKey(alleged_min_dep)) {
            spdlog::debug((boost::format{"[%1%] Minimum dependency: %2% (error=%3%)"} %
                                  recursion_depth_ % alleged_min_dep.ToString() % info->error_).str());
    ...
    spdlog::debug((boost::format{"* %1%/%2% alleged minimum dependencies might be non-minimal"} %
                          num_uncertain_min_deps % alleged_min_deps->GetSize()).str());
    ...
    spdlog::debug((boost::format{"* %1% alleged maximum non-dependencies (%2%)"} %
                          alleged_max_non_deps.size() % "UNIMPLEMENTED").str());
    ...
    for (auto& alleged_max_non_dep : alleged_max_non_deps) {
        ...
        spdlog::trace((boost::format{"* Alleged maximal non-dependency %1%: non-dep?: %2%, "
                                             "error: %3%"} %
                              alleged_max_non_dep.ToString() % is_non_dep % error).str());
        if (is_non_dep) {
    ...
    if (peaks.empty()) {
        ...
        spdlog::debug((boost::format{"* %1% new peaks (%2%)"} % peaks.size() % "UNIMPLEMENTED").str());
        ...
        spdlog::debug((boost::format{"* Increasing sampling boost factor to %1%"} % new_sample_boost).str());
...
std::optional<Vertical> SearchSpace::TrickleDownFrom(
    ...
    if (candidate_error <= strategy->max_dependency_error_) {
        spdlog::trace((boost::format{"* Found %1%-ary minimum dependency candidate: %2%"} %
                              min_dep_candidate.vertical_.GetArity() % min_dep_candidate).str());
        ...
        spdlog::trace((boost::format{"* Guessed incorrect %1%-ary minimum dependency candidate."} %
                              min_dep_candidate.vertical_.GetArity()).str());
