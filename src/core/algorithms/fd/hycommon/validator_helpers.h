...
void LogLevel(std::vector<VertexAndAgreeSet> const& cur_level_vertices,
    ...
    spdlog::info("LEVEL {} ({}): {} intersections; {} validations; {} invalid; {} new candidates; --> {} {}s",
        current_level_number, cur_level_vertices.size(),
              result.CountIntersections(),
              result.CountValidations(), num_invalid_instances,
              candidates, num_valid_instances,
              primitive);
}
