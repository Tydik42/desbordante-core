...
void StatsCalculator::VisualizeHighlights() const {
    for (auto const& highlight : highlights_) {
        spdlog::debug("- LHS value: {}, Size: {}, Number of different RHS values: {}, "
                      "Proportion of most frequent RHS value: {}",
                      GetLhsStringValue(highlight.GetCluster()[0]),
                      highlight.GetCluster().size(),
                      highlight.GetNumDistinctRhsValues(),
